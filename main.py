...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    amount = sum([int(a) for a in update.message.text.split() if a.isdigit()])
    ...
    for num in numbers:
        ledger[num] = ledger.get(num, 0) + 500
        user_data[user.username][key].append((num, 500))

    await update.message.reply_text(f"{sum(500 for _ in numbers)} လို")
